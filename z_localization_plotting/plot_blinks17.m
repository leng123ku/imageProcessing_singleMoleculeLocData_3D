function plot_blinks17(xyresolution, zresolution, pixsize, zdist, CCDpixelx, CCDpixely, directoryName, directoryName_cal, filePath_ZWxy, imagetitle, DateRec, RunNo, Rxy_Z_table, nimage, vispPath, depth, drawtype, frameGroup)
%%
...
for n_plot=2:2
    ...
    llimRxy = min(Rxy_Z_table(:,1));%min(Rxy_Z_appRange(:,1))-0.1;
    ulimRxy = max(Rxy_Z_table(:,1));% max(Rxy_Z_appRange(:,1));
    ...
    bestFittingCurve_option = 1; % 1: Yes, 2: No
    [nd,~] = size(position);
    ...
    if bestFittingCurve_option == 1
        ...
        while k_count < n_pos
            ...
            if (wX(j,1) < ulimw) && (wX(j,1) > llimw)
                if (wY(j,1) < ulimw) && (wY(j,1) > llimw)
                    if (R(j) < 1) && (R(j) > llimR)
        ...
        [Wx_Wy_Z] = determineBestWyWxCurve(selBlinks, Rxy_Z_table, directoryName);
    ...
    Zc(dX > ulimdx) = NaN; % 0.2 pixel ~ 25 nm
    crit_xres = sum(isnan(Zc)) - crit_total;
    crit_total = sum(isnan(Zc));
    
    Zc(dY > ulimdy) = NaN; % 0.2 pixel ~ 25 nm
    crit_yres = sum(isnan(Zc)) - crit_total;
    crit_total = sum(isnan(Zc));
    
    Zc(dZ > zresolution) = NaN; % 100
    ...
    subplot(624)
    [count, bins] = hist(dZ,30);
    bar(bins, count, 'hist');
    axis([0 100 0 max(count)]);
    xlabel('dZ (nm)', 'FontSize', 16);
    
    subplot(625);
    [count, bins] = hist(wX,50) ;
    bar(bins, count, 'hist');
    axis([1 3 0 max(count)]);
    xlabel('Wx (px)', 'FontSize', 16);
    
    subplot(626);
    [count, bins] = hist(dwX,50) ;
    bar(bins, count, 'hist');
    axis([0 0.5 0 max(count)]);
    xlabel('dWx (px)', 'FontSize', 16);
    
    subplot(627);
    [count, bins] = hist(wY,50) ;
    bar(bins, count, 'hist');
    axis([1 3 0 max(count)]);
    xlabel('Wy (px)', 'FontSize', 16);
    
    subplot(628);
    [count, bins] = hist(dwY,25) ;
    bar(bins, count, 'hist');
    axis([0 0.5 0 max(count)]);
    xlabel('dWy (px)', 'FontSize', 16);
    
    subplot(629);
    [count, bins] = hist(Rxy,50) ;
    bar(bins, count, 'hist');
    axis([0 3 0 max(count)]);
    xlabel('Rxy', 'FontSize', 16);
    
    subplot(6,2,10);
    [count, bins] = hist(Zc,25) ;
    bar(bins, count, 'hist');
    axis([0 1000  0 max(count)]);
    xlabel('Zc (nm)', 'FontSize', 16);
    
    subplot(6,2,11);
    [count, bins] = hist(R,20) ;
    bar(bins, count, 'hist');
    axis([llimR 1  0 max(count)]);
    ...
    subplot(1,2,1);
    xedges = linspace(1,2000,100); yedges = linspace(0,20,100);
    ...
    if Visp_output == 1
        ...
        for i = 1:nd
            ...
            if flag_visp == 0
                m3d=m3d+1;
                fprintf(fid_3dlp,'%7.0f \t %7.0f \t %7.0f \t %6.1f \t %6.1f \t %6.1f \t %6.2f \t %d \n',...
    ...
    ylabel('dwY');
    subplot(224)
    xedges = linspace(1,1500,100); yedges = linspace(0,150,100);
    ...
    cprintf('*black', '%d failed XY resolution criteria. \n', crit_xres + crit_yres);

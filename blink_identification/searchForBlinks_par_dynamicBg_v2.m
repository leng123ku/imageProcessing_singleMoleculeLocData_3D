function n_blinks_tot = searchForBlinks_par_dynamicBg_v2(n_file, directoryName, cut_xy, channel)
...
for i = 1:n_dev
    ...
    for k = from:to
        base{k} = base_temp;
        threshold{k} = threshold_mean + floor(2*threshold_sd);%threshold_mean+ 2*threshold_sd;
    end
end
...
for m = 1%1:num_image
    ...
    if flag_sum >= 1
        ...
        if (num_bead_afterMaxCheck >= 2)
            ...
            ref = zeros(2,num_bead_afterMaxCheck*(num_bead_afterMaxCheck-1)/2); % m*(m-1)/2
            p = 0; % column counter
            for k = 1:num_bead_afterMaxCheck-1
                for l = k+1:num_bead_afterMaxCheck
                    p = p+1;
                    ref([1 2],p) = [k l];
                end
            end
            dist = pdist([x_flag y_flag]);
            pt_to_check = find(dist <= neighCheck+3);
            %n_pt_to_check = size(pt_to_check,2);
            for s = 1:size(pt_to_check,2)
                id_1 = ref(1,pt_to_check(s));
                id_2 = ref(2,pt_to_check(s));
                
                if A(x_flag(id_1),y_flag(id_1)) > A(x_flag(id_2),y_flag(id_2))
                    flag(x_flag(id_2),y_flag(id_2))=  false;
                else
                    flag(x_flag(id_1),y_flag(id_1))= false;
                end
            end

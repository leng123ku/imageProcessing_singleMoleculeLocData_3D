function [position_fit]  = fit_blinks_fitFunc(directoryName)
...
fitFunc2 = fittype( 'a+b*((erf((x-c+0.5)/d)-erf((x-c-0.5)/d)).*(erf((y-e+0.5)/f)-erf((y-e-0.5)/f)))',...
...
%                     R = 1 - sum(Res.^2)/totalRes;
                    ...
                    [fitCoeff,gof,output] = fit(x,ff,fitFunc2,...
                        'Lower',[0,1,coeff(3)-2,0.5,coeff(5)-2,0.5],...
                        'Upper',[Inf,Inf,coeff(3)+2,5,coeff(5)+2,5],...
                        'Startpoint',[coeff(1) coeff(2) coeff(3) coeff(4) coeff(5) coeff(6)],...
                        'Display','off',...
                        'MaxIter',100,... %100
                        'TolFun',1e-6,... %1e-2
                        'TolX',1e-6);     %1e-2
                    
                    cfit = coeffvalues(fitCoeff);
                    cfit(4) = cfit(4)/sqrt(2);
                    cfit(6) = cfit(6)/sqrt(2);
                    ci = confint(fitCoeff,0.68);
                    dc = (ci(2,:)-ci(1,:))/2;
                    dc(4) = dc(4)/sqrt(2);
                    dc(6) = dc(6)/sqrt(2);
                    R = gof.rsquare;

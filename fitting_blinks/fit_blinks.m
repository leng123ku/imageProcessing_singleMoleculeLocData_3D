...
t_estimate = ceil(nOfBlinks/770/60);
t_now = clock;
...
%% Fitting
noiselevel = 2;
opts = optimset('TolFun',1e-4,'TolX',1e-4,'MaxIter', 50);
%opts = optimset('TolFun',1e-10,'TolX',1e-10,'MaxIter', 1000);
fitFunc2 = @(c, surface) ...
...
%     end
    ...
    parfor k = 1:n1
        ...
        %dx = 2; dy = 2;
        ff = f(:);
        if (dx > 0.4) && (dy > 0.4)  %0.5   0.5
            if (dx < 12) && (dy < 12)  % 12 12
                %if ~((dx > 8) && (dy > 8))
                    a = a*dx*dy;
                    coeff = [1, a, x0, dx, y0, dy];
